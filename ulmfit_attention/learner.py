...
def get_text_classifier_custom(enc_arch: Callable, head_class: str, head_params: Dict, vocab_sz: int, n_class: int,
    ...
    head_params['dv'] = dv
    pooling = Aggregation.factory(head_class, head_params)
...
def text_classifier_learner_custom(data: DataBunch, enc_arch: Callable, head_class: str, head_params,
    ...
    model = get_text_classifier_custom(enc_arch, head_class, head_params, len(data.vocab.itos), data.c,
                                       **classifier_params)
    meta = _model_meta[enc_arch]
